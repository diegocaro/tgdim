...
unsigned bufferxor[BUFFERSIZE];


PRB2XORCompactQtree::PRB2XORCompactQtree(vector<Point<uint> > &vp,
                               BitSequenceBuilder *bs, BitSequenceBuilder *bb, BitSequenceBuilder *bc,XorCodeBuilder *xorbuild,int k1, int k2, int F, int max_level_k1, int max_levels_ki)
: k1_(k1), k2_(k2), F_(F) {
...
//    vp.erase(last, vp.end());
    ...
    create(vp, bs, bb, bc, xorbuild);
}

//void PRB2XORCompactQtree::get_stats(const std::vector<Point<uint> > &vp) {
//    struct Node z;
//    queue<struct Node> q;
...
void PRB2XORCompactQtree::get_stats(std::vector<Point<uint> > &vp) {
    struct Node z;
    queue<struct Node> q;
    size_t r[max_children_ + 1];
    ...
    while (!q.empty()) {
       ...
       int bin = 0;
       for (size_t i = z.lo; i < z.hi; ++i) {
           if(i%1000000==0) fprintf(stderr,"Progress step1: %.2f\r",1.0*((z.level)*vp.size()+i)/(depth_*vp.size())*100);
...
void PRB2XORCompactQtree::create(const std::vector<Point<uint> > &vp,
                            BitSequenceBuilder *bs, BitSequenceBuilder *bb, BitSequenceBuilder *bc, XorCodeBuilder *xorbuild) {
    struct Node z;
    queue<struct Node> q;
    size_t r[max_children_ + 1];
    ...
    vector< vector < vector<unsigned> > > leaves;
    for(int i = 0; i < depth_-1; i++) {
        vector< vector<unsigned> > t1;

        for(int j = 0; j < num_dims_; j++) {
            vector<unsigned> t2(leaves_per_level_[i],0);
            t1.push_back(t2);
        }
        leaves.push_back(t1);
    }
    ...
    while (!q.empty()) {
        ...
        for(int i = 0; i < children_[z.level]+1; i++) {
             ...
             // Count.
             for (size_t i = z.lo; i < z.hi; ++i) {
                 if(i%1000000==0) fprintf(stderr,"Progress step2: %.2f\r",1.0*((z.level)*vp.size()+i)/(depth_*vp.size())*100);
                 start[code(vp[i],z.level)] += 1;
             }
...
//            r[j] = rank(vp, j, z.level, z.lo, z.hi - 1);
            ...
            if (r[j] - r[j - 1] > 0) {
                ...
                if (z.level < depth_ - 1) {
                  ...
                  if (r[j]-r[j-1] <= F_) { //was F
                    ...
                    cds_utils::bitset(countbits[z.level], curr_pos[z.level] + r[j]-r[j-1] - 1);
                    ...
                    for(size_t jj = r[j-1]; jj < r[j]; jj++) {
                        ...
                        if (false == is_interleaved_) {
                          ...
                          for(int i=0; i < num_dims_; i++) {
                            leaves[z.level][i][curr_pos[z.level]] = p[i] % maxval;
                            //printf(" %u", p[i] % maxval);
                          }
                            //printf("\n");
                        }
    ...
    //                                  printf("level %d storing %u (dimc %d) %d\n",z.level, p[i] % maxvalb,i,p[i]);
                                    leaves[z.level][i][curr_pos[z.level]] =  p[i] % maxvalb;
                                  }
                              }
                              else {
                                  for(int i=0; i < num_dims_; i++) {
...
void PRB2XORCompactQtree::all(Point<uint> p, size_t z, int level, vector<Point<uint> > &vpall) {
    size_t y = 0;
...
void PRB2XORCompactQtree::range(Point<uint> &p, size_t z, int level, Point<uint> &from, Point<uint> &to, vector<Point<uint> > &vpall,size_t &items, bool pushval) {
    if (level != -1 ) {
        Point<uint> hi(p);
        if (false == is_interleaved_) {
            hi += nk_[level];
...
void PRB2XORCompactQtree::stats_space() const {
  size_t treebits=0;
  size_t leafbits=0;
  size_t arraysize=0;
  size_t countbits=0;
  ...
  printf("%10s\t%10s\t%8s\t%10s\t%8s\t", "|C|","C.1s","C.1s/|C|", "C.bytes","C.cratio");
  printf("%10s\n", "X.bytes");
  for(int i = 0; i < depth_; i++) {
  ...
  uint type = loadValue<uint>(f);
   ...
   leaves_ = new XorCode**[depth_];
     ...
     for(int i = 0; i < depth_-1; i++) {
         hc_[i] = new MyCoder*[num_dims_];
         for(int j=0; j < num_dims_; j++) {
             hc_[i][j] = MyCoder::load(f);
         }
     }

    }

void PRB2XORCompactQtree::save(ofstream &f) const {
  uint wr = PRQBLK2XOR_SAV;
...
void PRB2XORCompactQtree::print_leaves() {
...
// void PRB2XORCompactQtree::print_leaves(Point<uint> p, size_t z, int level) {
//     size_t y = 0;
//
//     if (level == depth_-1) {
//
...
/*
 template <typename T>
 void PRBCompactQtree<T>::save(ofstream & f) const
 {
 uint wr = MXQTRE_SAV;
 saveValue(f,wr);

 saveValue< PRBCompactQtree<T> >(f, this, 1);
 T_->save(f);
 }

 template <typename T>
 PRBCompactQtree<T>* PRBCompactQtree<T>::load(ifstream & f) {
 assert(f.good());
 uint type = loadValue<uint>(f);
 if(type!=MXQTRE_SAV) {	 // throw exception
 abort();
 }
 PRBCompactQtree<T> * ret = loadValue< PRBCompactQtree<T> >(f, 1);
