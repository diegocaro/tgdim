...
#ifndef PRWCOMPACTQTREE_H_
#define PRWCOMPACTQTREE_H_
...
//template<typename T>
class PRWCompactQtree:public CompactQtree {
  struct Node {
      size_t lo, hi;
      int level;
  };
    ...
    struct Less {
    Less(const PRWCompactQtree& cl) : c(cl) {
//                printf("depth: %d\n",c.depth_);
...
//                }
 ...
 public:
    ...
    PRWCompactQtree(std::vector<Point<uint> > &points, BitSequenceBuilder *bs, BitSequenceBuilder *bb, int k1=2, int k2=2, int max_level_k1=0, int max_levels_ki=0);

    virtual ~PRWCompactQtree() {
        __setdefaultvalues();
    }
    ...
    //static PRWCompactQtree* load(ifstream & f);
 ...
 protected:
    ...
    vector<size_t> graywhite_; //number of bits set (gray or white nodes) per level
    vector<size_t> leaves_per_level_;
