...
PRB2CompactQtree::PRB2CompactQtree(vector<Point<uint> > &vp,
                               BitSequenceBuilder *bs, BitSequenceBuilder *bb, BitSequenceBuilder *bc,int k1, int k2, int F, int max_level_k1, int max_levels_ki)
: k1_(k1), k2_(k2), F_(F) {
...
//void PRB2CompactQtree::get_stats(const std::vector<Point<uint> > &vp) {
//    struct Node z;
//    queue<struct Node> q;
...
void PRB2CompactQtree::all(Point<uint> p, size_t z, int level, vector<Point<uint> > &vpall) {
    size_t y = 0;
...
void PRB2CompactQtree::range(Point<uint> &p, size_t z, int level, Point<uint> &from, Point<uint> &to, vector<Point<uint> > &vpall,size_t &items, bool pushval) {
    if (level != -1 ) {
        Point<uint> hi(p);
        if (false == is_interleaved_) {
            hi += nk_[level];
...
void PRB2CompactQtree::print_leaves(Point<uint> p, size_t z, int level) {
    size_t y = 0;
